...
import { debugMode, log } from './vars.js';
...
export class ProgressTracker {
    ...
    constructor() {
        ...
        this.debugMode = debugMode; // Whether to use debug mode
        this.updateThreshold = 100; // Minimum time between updates in ms
        this.lastProgressLine = ''; // Store the last progress line
        
        if (this.debugMode) {
            log.debug('Progress tracker initialized in debug mode');
        }
    }
    ...
    setTotalFiles(total) {
        ...
        if (this.debugMode) {
            log.debug(`Total files set to: ${total}`);
        } else {
            this.updateProgress();
        }
    }
    ...
    incrementProcessed() {
        ...
        // In debug mode, log progress less frequently
        if (this.debugMode) {
            // Only log every 500 files in debug mode to prevent excessive output
            if (this.processedFiles % 500 === 0 || this.processedFiles === this.totalFiles) {
                log.debug(`Processed ${this.processedFiles} of ${this.totalFiles} files (${((this.processedFiles / this.totalFiles) * 100).toFixed(1)}%)`);
            }
        } else {
            this.updateProgress();
        }
     ...
     */
    updateProgress() {
        // Skip progress bar updates in debug mode
        if (this.debugMode) {
            return;
        }
        ...
        const now = Date.now();
        if (now - this.lastUpdateTime < this.updateThreshold && this.processedFiles < this.totalFiles) {
            return;
        }
        ...
        if (!this.initialized) {
            console.log(''); // Add a blank line before progress starts
        ...
        const progressLine = `${chalk.cyan('Progress:')} ${progressBar} ${chalk.yellow(`${percentage}%`)} | ` +
    ...
    complete() {
        ...
        if (this.debugMode) {
            log.debug(`Progress tracking completed: ${this.processedFiles}/${this.totalFiles} files processed in ${totalTime.toFixed(1)} seconds`);
        }
